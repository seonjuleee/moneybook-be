...
export class ExpenseService {
  ...
  private categorizeExpense(item: string, text: string): ExpenseCategory {
    ...
    // 식비 관련
    if (lowerText.includes('스타벅스') || lowerText.includes('카페') || 
        lowerText.includes('음식') || lowerText.includes('식당') ||
        lowerText.includes('배달') || lowerText.includes('점심') ||
        lowerText.includes('저녁') || lowerText.includes('아침')) {
      return ExpenseCategory.FOOD;
    }

    // 교통비 관련
    if (lowerText.includes('버스') || lowerText.includes('지하철') ||
        lowerText.includes('택시') || lowerText.includes('기차') ||
        lowerText.includes('주차') || lowerText.includes('주유')) {
      return ExpenseCategory.TRANSPORT;
    }

    // 쇼핑 관련
    if (lowerText.includes('마트') || lowerText.includes('편의점') ||
        lowerText.includes('백화점') || lowerText.includes('쇼핑') ||
        lowerText.includes('옷') || lowerText.includes('신발')) {
      return ExpenseCategory.SHOPPING;
    }

    // 문화생활
    if (lowerText.includes('영화') || lowerText.includes('공연') ||
        lowerText.includes('게임') || lowerText.includes('놀이공원')) {
      return ExpenseCategory.ENTERTAINMENT;
    }

    // 의료비
    if (lowerText.includes('병원') || lowerText.includes('약') ||
        lowerText.includes('치과') || lowerText.includes('의료')) {
      return ExpenseCategory.MEDICAL;
    }

    // 교육비
    if (lowerText.includes('학원') || lowerText.includes('책') ||
        lowerText.includes('교육') || lowerText.includes('강의')) {
      return ExpenseCategory.EDUCATION;
    }

    // 공과금
    if (lowerText.includes('전기') || lowerText.includes('수도') ||
        lowerText.includes('가스') || lowerText.includes('인터넷') ||
